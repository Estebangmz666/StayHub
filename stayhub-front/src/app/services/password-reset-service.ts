...
export class PasswordResetService {
  ...
  requestPasswordReset(email: string): Observable<void> {
    ...
    return this.http.post<void>(`${this.baseUrl}/users/request-password-reset`, requestData)
      .pipe(
        tap(() => {
  ...
  resetPassword(token: string, newPassword: string): Observable<SuccessResponseDTO> {
    ...
    return this.http.post<SuccessResponseDTO>(`${this.baseUrl}/users/reset-password`, resetData)
      .pipe(
        tap(response => {
